...
windows_on_spaces () {
  ...
  do
    ...
    do
      ...
      args+=(--set space.$space label="$icon_strip" label.drawing=on label.padding_right=$pad_right icon.padding_right=$icon_pad_right icon.padding_left=$icon_pad_left)
    done
  done <<< "$CURRENT_SPACES"
